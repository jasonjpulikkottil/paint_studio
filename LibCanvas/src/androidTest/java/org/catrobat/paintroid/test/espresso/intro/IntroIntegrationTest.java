package com.jdots.paint.test.espresso.intro;
...
public class IntroIntegrationTest {
	...
	public void testOnSkipPressedActivityFinished() {
		onView(withId(R.id.paint_studio_btn_skip)).perform(click());
	...
	public void testOnLetsGoPressedActivityFinished() {
		...
		for (int i = 0; i < adapter.layouts.length - 1; i++) {
			onView(withId(R.id.paint_studio_btn_next)).perform(click());
		}
	...
	public void testIntroToolsPageShowDescriptionOnPress() {
		...
		for (int layout: adapter.layouts) {
			if (layout == R.layout.paint_studio_slide_intro_tools_selection) {
				break;
			}
			onView(withId(R.id.paint_studio_btn_next)).perform(click());
		}

		onView(withId(R.id.paint_studio_textview_intro_tools_header))
	...
	public void testIntroViewPagerSwipeChangePage() {
		onView(withId(R.id.paint_studio_intro_welcome_head))
				.check(matches(isDisplayed()));

		onView(withId(R.id.paint_studio_view_pager)).perform(swipeLeft());

		onView(withId(R.id.paint_studio_intro_possibilities_head))
				.check(matches(isDisplayed()));
		...
		onView(withId(R.id.paint_studio_intro_landscape_head))
				.check(matches(isDisplayed()));

		onView(withId(R.id.paint_studio_view_pager)).perform(swipeLeft());

		onView(withId(R.id.paint_studio_intro_started_head))
				.check(matches(isDisplayed()));
	}
