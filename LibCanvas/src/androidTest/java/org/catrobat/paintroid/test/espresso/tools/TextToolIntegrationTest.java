...
import com.jdots.paint.tools.implementation.TextTool;
...
public class TextToolIntegrationTest {
	...
	public void setUp() {
		...
		textEditText = activity.findViewById(R.id.paint_studio_text_tool_dialog_input_text);
		fontSpinner = activity.findViewById(R.id.paint_studio_text_tool_dialog_spinner_font);
		underlinedToggleButton = activity.findViewById(R.id.paint_studio_text_tool_dialog_toggle_underlined);
		italicToggleButton = activity.findViewById(R.id.paint_studio_text_tool_dialog_toggle_italic);
		boldToggleButton = activity.findViewById(R.id.paint_studio_text_tool_dialog_toggle_bold);
		textSize = activity.findViewById(R.id.paint_studio_font_size_text);
	...
	public void testTextToolStillEditableAfterClosingTextTool() {
		...
		onView(withId(R.id.paint_studio_text_tool_dialog_input_text)).perform(replaceText(TEST_TEXT_ADVANCED));
	...
	public void testDialogKeyboardTextBoxAppearanceOnStartup() {
		onView(withId(R.id.paint_studio_text_tool_dialog_input_text)).check(matches(hasFocus()));
		checkTextBoxDimensionsAndDefaultPosition();
	}

	@Test
	public void testDialogDefaultValues() {
		onView(withId(R.id.paint_studio_text_tool_dialog_input_text))
		...
		onView(withId(R.id.paint_studio_text_tool_dialog_toggle_underlined))
				.check(matches(isNotChecked()));
		onView(withId(R.id.paint_studio_text_tool_dialog_toggle_bold))
				.check(matches(isNotChecked()));
		onView(withId(R.id.paint_studio_text_tool_dialog_toggle_italic))
				.check(matches(isNotChecked()));
	...
	public void testInputTextAndFormatForTextSize50() {
		...
		float boxWidth = getToolMemberBoxWidth();
		float boxHeight = getToolMemberBoxHeight();
		onView(withId(R.id.paint_studio_font_size_text)).perform(replaceText("50"));
		checkTextBoxDimensions();
	...
	public void testInputTextAndFormatForTextSize100() {
		...
		float boxWidth = getToolMemberBoxWidth();
		float boxHeight = getToolMemberBoxHeight();
		onView(withId(R.id.paint_studio_font_size_text)).perform(replaceText("100"));
		checkTextBoxDimensions();
		assertTrue("Text box width should be larger with bigger text size", getToolMemberBoxWidth() > boxWidth);
		assertTrue("Text box height should be larger with bigger text size", getToolMemberBoxHeight() > boxHeight);
	}
	...
	public void testInputTextAndFormatForTextSize300() {
		...
		float boxWidth = getToolMemberBoxWidth();
		float boxHeight = getToolMemberBoxHeight();
		onView(withId(R.id.paint_studio_font_size_text)).perform(replaceText("300"));
		checkTextBoxDimensions();
		assertTrue("Text box width should be larger with bigger text size", getToolMemberBoxWidth() > boxWidth);
		assertTrue("Text box height should be larger with bigger text size", getToolMemberBoxHeight() > boxHeight);
	}
	...
	private void enterTextInput(final String textToEnter) {
		 ...
		 *
		 * See:
		 * java.ic_paint_studio_menu_language.RuntimeException: Failed to get key events for string السلام عليكم 123 (i.e.
		 ...
		 */
		onView(withId(R.id.paint_studio_text_tool_dialog_input_text)).perform(replaceText(textToEnter));
		Espresso.closeSoftKeyboard();
		onView(withId(R.id.paint_studio_text_tool_dialog_input_text)).check(matches(withText(textToEnter)));
	}
	...
	private void selectFormatting(FormattingOptions format) {
		switch (format) {
			...
			case STC:
			case DUBAI:
				onView(withId(R.id.paint_studio_text_tool_dialog_spinner_font)).perform(click());
